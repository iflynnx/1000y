   ...
   case Msg of
      ...
      FM_SAY:
         begin
            ...
            if ReverseFormat (str, '%s: Ω´√≈≈…Œ‰π¶¥´ ⁄∏¯%s', sayer, objectname, gradename, 2) then begin
               if boMagicNpc then begin
                  ...
                  if Bo = nil then begin BocSay (format ('%s≤ª‘⁄.',[objectname])); exit; end;
                  if not (Bo is TUser) then begin BocSay (format ('%s≤ª « π”√’ﬂ.',[objectname])); exit; end;
                  if TUser(Bo).GuildName = '' then begin BocSay ('≤ª «√≈≈…≥…‘±.'); exit; end;
                  if TUser(Bo).GuildName <> TGuildObject(ParentGuildObject).GuildName then begin
                     BocSay (format ('%s“—”µ”–√≈≈….',[objectname]));
                     exit;
                  end;
                  if StrPas (@GuildMagicData.rName) = '' then begin BocSay ('√ª”–√≈≈…Œ‰π¶.'); exit; end;
                  if GuildMagicData.rcSkillLevel < 5000 then begin BocSay ('√≈≈…Œ‰π¶–ﬁ¡∂÷µ≤ª◊„.'); exit; end;
                  ...
                  end else BocSay ('√≈≈…Œ‰π¶ ⁄»® ß∞‹.');
               end;
               exit;
            end;

            if ReverseFormat (str, '%s: %sµƒ√˚◊÷ «%s', sayer, objectname, gradename, 3) then begin
               ...
               if objectname = GuildNpcName then begin
                  ...
                  BocChangeProperty;
                  BocSay (format ('Œ“µƒ√˚◊÷≥…¡À%s.',[GuildNpcName]));
                  exit;
               end;
               exit;
            end;

            if ReverseFormat (str, '%s: %sµƒŒª÷√ «’‚¿Ô', sayer, objectname, gradename, 2) then begin
               // objectname := copy (objectname, 1, Length(objectname)-2);
               if objectname = GuildNpcName then begin
            ...
            if ReverseFormat (str, '%s: %s∏˙◊≈¿¥', sayer, objectname, gradename, 2) or
               ReverseFormat (str, '%s: %s∏˙◊≈¿¥', sayer, objectname, gradename, 2) then begin
               // objectname := copy (objectname, 1, Length(objectname)-2);
               if objectname = GuildNpcName then begin
            ...
            if ReverseFormat (str, '%s: %sÕ£÷π¡À', sayer, objectname, gradename, 2) or
               ReverseFormat (str, '%s: %sÕ£÷π¡À', sayer, objectname, gradename, 2) then begin
               // objectname := copy (objectname, 1, Length(objectname)-2);
            ...
            if ReverseFormat (str, '%s: %s–›œ¢œ¬', sayer, objectname, gradename, 2) or
               ReverseFormat (str, '%s: %s–›œ¢œ¬', sayer, objectname, gradename, 2) then begin
               // objectname := copy (objectname, 1, Length(objectname)-2);
               if objectname = GuildNpcName then begin
                  if BasicData.Feature.rfeaturestate = wfs_sitdown then begin
                     BasicData.Feature.rfeaturestate := wfs_normal;
